...
const generateCommentsForNews = async (newsItem, communityUsers, expertUsers) => {
  ...
  // Select 12 in-favor comments and 8 against comments
  const inFavorComments = getRandomItems(commentTemplates.inFavor, 12);
  const againstComments = getRandomItems(commentTemplates.against, 8);
  
  // Process in-favor comments
  for (let i = 0; i < inFavorComments.length; i++) {
    const template = inFavorComments[i];
  ...
  // Process against comments
  for (let i = 0; i < againstComments.length; i++) {
    const template = againstComments[i];
    const commenter = getRandomItem(communityUsers);
  ...
  try {
    ...
    for (let i = 0; i < realNewsData.length; i++) {
      ...
      for (const commentData of comments) {
        try {
          // Remove non-schema fields before saving
          const { groupName, stance, ...dbCommentData } = commentData;
          
          const comment = new CommunityComment(dbCommentData);
          ...
          insertedComments.push(fullComment);
          console.log(`  ✓ Added comment: ${stance} - ${groupName}`);
        } catch (error) {
      ...
      // Count stances
      const inFavorCount = insertedComments.filter(c => c.stance === 'in_favor').length;
      const againstCount = insertedComments.filter(c => c.stance === 'against').length;
      
      totalInFavor += inFavorCount;
      totalAgainst += againstCount;

      // Group comments by category
      const groupedComments = {};
      insertedComments.forEach(comment => {
        if (!groupedComments[comment.groupName]) {
          groupedComments[comment.groupName] = [];
        }
        groupedComments[comment.groupName].push(comment);
      });
      ...
      const newsResults = {
        ...
        inFavorComments: inFavorCount,
        againstComments: againstCount,
      ...
      const filename = `news_${i + 1}_comments_corrected.json`;
      fs.writeFileSync(
      ...
      console.log(`  📊 In favor: ${inFavorCount}, Against: ${againstCount}`);
    }

    // Save complete results
    fs.writeFileSync(
      path.join(__dirname, 'all_news_comments_corrected.json'),
    ...
    const summary = {
      ...
      totalInFavorComments: totalInFavor,
      totalAgainstComments: totalAgainst,
      communityUsersUsed: communityUsers.length,
      expertUsersUsed: expertUsers.length,
      filesGenerated: realNewsData.length + 2,
      averageCommentsPerNews: 20,
      commentDistribution: {
        inFavorPerNews: 12,
        againstPerNews: 8
      }
    ...
    fs.writeFileSync(
      path.join(__dirname, 'comment_generation_corrected_summary.json'),
